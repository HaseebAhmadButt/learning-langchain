...
# TODO: Langsmith Followup: https://smith.langchain.com/o/aff64684-7af9-4e1e-9ee5-2755b5e0273d/projects/p/3b201557-0d71-411f-b573-3cf99bc30125?onboarding=Testing+Tracing&timeModel=%7B%22duration%22%3A%221d%22%7D
...
def main():
    ...
    # llm = ChatOpenAI(model="gpt-5", temperature=0) # Replaced with OLLAMA API, which is a local LLM API that can be used to interact with local language models. The model parameter specifies the name of the model to use, and the temperature parameter controls the randomness of the responses.
    llm = ChatOllama(model="qwen3-coder:30b", temperature=0) # Replaced with OLLAMA API, which is a local LLM API that can be used to interact with local language models. The model parameter specifies the name of the model to use, and the temperature parameter controls the randomness of the responses.
